...
macro_rules! proxy_data_mut {
    ($data:expr) => {
        if let Some(data) = ($data as &mut dyn std::any::Any).downcast_mut() {
            ...
            ::syrillian_utils::debug_panic!("Could not downcast proxy data. The GPU data type did not match up.");
            return;
        }
    };
}
...
impl SceneProxyBinding {
    pub fn new(component_id: TypedComponentId, local_to_world: Affine3<f32>, proxy_data: Box<dyn Any>, proxy: Box<dyn SceneProxy>) -> Self {
        Self {
            component_id,
            local_to_world,
            proxy_data,
    ...
    pub fn update(&mut self, renderer: &Renderer, window: &Window) {
        self.proxy.update_render(renderer, self.proxy_data.as_mut(), window, &self.local_to_world.matrix());
    }

    pub fn render(&self, renderer: &Renderer, ctx: &GPUDrawCtx) {
        self.proxy.render(renderer, self.proxy_data.as_ref(), ctx, &self.local_to_world.matrix())
    }
}